...
from model.utils.funcs import RecurringList, compute_consecutive_distances, find_indices, sample_items, reorder_list, \
    remove_duplicates
from model.utils.all_prompts import get_start_point_prompt, get_dayplan_prompt, get_system_prompt, get_hour_prompt, \
...
class ItiNera:
    ...
    def save_qualitative(self, new_numerical_order, full_response, clusters, lookup_dict):
        ...
        def create_map(df, polyline_color="green", map_filename=""):
            ...
            for i, (_, row) in enumerate(df.iterrows()):
                html = f"""
                    <div style="width:300px; height:350px; overflow-y:auto; overflow-x:hidden; font-size:14px;">
                        <img src="{row['picture']}" style="width:100%; height:auto; margin-bottom:10px;"><br>
                        <b>{i} - {row['name']}</b><br>
                        {row['context']}<br>
                        门票价格为：{row['price']}<br>
                        营业时间为：{row['time']}<br>
                        携程参考链接为：<a href="{row['link']}" target="_blank">点击访问</a>
                    </div>
                """
                iframe = IFrame(html=html, width=320, height=370)  # 高度加大，容纳图片和文字
        ...
        full_tsp_map_path = f'./model/output/{current_time}_fulltsp.html'
        ...
        response_map_path = f'{current_time}.html'
        ...
        food_info = pd.read_csv(r"model/data/food.csv", encoding="utf-8")
        hotel_info = pd.read_csv(r"model/data/hotel.csv", encoding="utf-8")

        K = 2

        nearest_food_details = []
        nearest_hotel_details = []

        for lat, lon in points:
            food_info['distance'] = abs(food_info['lat'] - lat) + abs(food_info['lon'] - lon)
            hotel_info['distance'] = abs(hotel_info['lat'] - lat) + abs(hotel_info['lon'] - lon)

            nearest_food = food_info.nsmallest(K, 'distance')
            nearest_hotel = hotel_info.nsmallest(K, 'distance')

            food_descriptions = [
                f"{row['名称']}  |  地址：{row['省份']}{row['市县区']}{row['区域']}{row['地址']}  |  人均价格：{row['人均价格']}"
                for _, row in nearest_food.iterrows()
            ]

            hotel_descriptions = [
                f"{row['name']}  |  星级：{row['rank']}"
                for _, row in nearest_hotel.iterrows()
            ]

            nearest_food_details.append(food_descriptions)
            nearest_hotel_details.append(hotel_descriptions)

        print("第一个点最近的K个美食（带名称地址）：")
        print(nearest_food_details[0])

        print("\n第一个点最近的K个酒店（带名称地址）：")
        print(nearest_hotel_details[0])
        ...
        return {
            ...
            "full_tsp_map_path": full_tsp_map_path,
            "food_detail": nearest_food_details,
            "hotel_detail": nearest_hotel_details
        }
    ...
    def calculate_ordered_route_info(self, new_numerical_order):
        ...
        if self.start_poi is not None and self.start_poi in new_numerical_ordered_poiname:
            ...
            msg = [{"role": "user", "content": get_start_point_prompt(candidate_points=new_numerical_ordered_poiname,
        ...
        msg = [
            {"role": "user", "content": check_final_reverse_prompt(context=context_string, user_reqs=self.user_reqs)}]
        response = self.proxy.chat(messages=msg, model=self.MODEL).replace("'", '"')
        ...
        for i, (id, name) in enumerate(zip(new_numerical_order, self.user_favorites.loc[new_numerical_order, "name"])):
            lookup_dict[i + 1] = [self.r2i[id],
                                  name]  # this ensures the real id from the database could be obtained through this lookup
    ...
    def get_full_order(self, all_pairs: list, clusters: list, clusters_order: np.ndarray):
        ...
        for i, cluster_order in enumerate(clusters_order):
            ...
            if i == 0:
                ...
                ordered_poi_names_within_cluster = np.array(
                    self.user_favorites.loc[ordered_pois_within_cluster, "name"].tolist())
                if len(cluster) < 2:
                ...
                if len(cluster) == 2:
                    ...
                    _, order_within_cluster = self.spatial_handler.solve_tsp_with_start_end(dist_matrix, start_poi,
                                                                                            end_poi)
    ...
    def solve(self):
        ...
        all_pairs, clusters, clusters_order = self.get_poi_candidates(req_topk_pois, self.must_see_pois,
                                                                      pseudo_must_see_pois)
        new_numerical_order, lookup_dict, clusters, context_string, must_see_string = self.get_full_order(all_pairs,
                                                                                                          clusters,
                                                                                                          clusters_order)
        full_response = self.get_day_plan(new_numerical_order, context_string, must_see_string)
        data = self.save_qualitative(new_numerical_order, full_response, clusters, lookup_dict)
        full_response_data = data["full_response"]
        path = data["response_map_path"]
        food_detail = data["food_detail"]
        hotel_detail = data["hotel_detail"]
        time = self.hours

        print(f'行程: \n{full_response_data}')
        return full_response_data, lookup_dict, path, time,food_detail,hotel_detail

def get_system_prompt(maxPoiNum, numMustSee, numCandidates):
    ...
    system_prompt = f"""
    您好Deepseek, 请扮演一个旅行大师，精通各种精致旅行体验。你的任务是从给出的'潜在兴趣点'列表中，制定完美的一日行程。你的特点？通过编织生动的故事和唤起一种如此强烈的漫游欲望，以至于仅仅通过听到你的故事就感觉如身临其境。
...
def check_final_reverse_prompt(context, user_reqs):
    prompt = f"""
    您好Deepseek，这里有一个列表或者一段文本描述一些场所或者点。请帮我判断其中大部分酒吧是处于前半部分还是后半部分，以一个 JSON list 返回判断结果。
...
def get_hour_prompt(user_reqs):
    ...
    prompt = f"""
    您好Deepseek,请扮演一个顶级AI时间规划助手，助手的工作是根据用户需求，来判断用户完成需求所需的一日行程时间。如果用户需求为空，请默认返回 ["4"]。
...
def get_start_point_prompt(candidate_points, user_reqs, return_candidates, distance_string):
    ...
    prompt = f"""
    您好Deepseek,请扮演一个顶级AI旅行规划助手，助手的工作是根据用户需求和提供的候选点，返回一日游行程的最佳起点的索引。如果候选点为空，请默认返回 ["0"]。
...
def process_input_prompt(user_input):
    ...
    prompt = f"""
    您好Deepseek，请帮我基于一段用户需求描述，将其拆解成多个独立的需求，每个独立的需求应该包括正向需求和负向需求。请直接根据给出的**用户输入**返回按照下面的格式的结果，不需要编写任何代码。
...
def get_dayplan_prompt(context_string, must_see_string, keyword_reqs, userReqList, maxPoiNum, numMustSee, numCandidates, itinerary_reqs, start_end, comments="", hours=None, mark_citywalk=False):
    ...
    prompt = f"""
    您好Deepseek，我邀请您成为一位极富创意和专业知识的旅游指南，专门为一天的行程设计出完美的旅游计划。
    ...
    6. 市区外景区的数量不能超过两个，而且要符合景点的**候选景点顺序**。**较远的行程应该放在行程末尾，千佛山则不应该是行程的最后一站**。
    7. 请确保用户的**关键词需求**是被严格满足的，比如用户如果提到了想去3个点，那么你规划的行程中应该完全按照用户的需求，只包含3个景点。
